...
// 2. Website Owner (info@nikamscientific.com)
// 3. Technical Partner (integralwebsolution@gmail.com)
export async function sendContactEmail(data: ContactFormData) {
  const transporter = await getTransporter();

  const ownerEmail = process.env.OWNER_EMAIL || process.env.CONTACT_EMAIL_TO || "info@nikamscientific.com";
  ...
  const [customerResult, ownerResult, agencyResult] = await Promise.allSettled([
    ...
    // 2. Email to Website Owner (info@nikamscientific.com)
    transporter.sendMail({
      from: fromEmail,
      to: ownerEmail,
        ...
        <td align="center">
          <div class="main">
            ...
            <div class="footer">
              ...
              Recipient: ${recipientRole === "owner" ? "info@nikamscientific.com" : "integralwebsolution@gmail.com"} &bull; Destination: nikamscientific.com
            </div>
          </div>
        </td>
      </tr>
